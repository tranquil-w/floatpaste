...
复制过的内容自动入库；随时按快捷键唤出速贴面板，**不离开当前输入场景**就能选一条重新上屏。文本、图片、文件路径都支持，图片可以按原样或以文件路径粘贴。

## ✨ 核心特性

- **无焦点速贴**：全局快捷键唤起面板，不抢占当前窗口焦点；数字直选 + 方向键导航，上屏后自动回到原窗口继续工作
- **图片与文件**：缩略图预览、悬停大图、哈希去重；图片条目支持「按原样粘贴」与「粘贴为文件路径」两种方式
...
| 键 | 作用 |
|----|------|
| `Alt+Q` | 唤起 / 关闭速贴面板（可自定义） |
| `Alt+S` | 唤起搜索窗口（可自定义，可停用） |
| `↑` `↓` / `1-9` | 速贴内导航 / 数字直达并上屏 |
| `Enter` | 上屏选中条目 |
| `Shift+Enter` | 图片条目按文件路径上屏 |
| `Ctrl+Enter` | 进编辑器 |
| `Ctrl+Space` | 收藏 / 取消收藏 |
| `Esc` | 关闭并归还焦点 |
