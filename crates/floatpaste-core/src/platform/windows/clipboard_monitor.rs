...
/// 剪贴板录入回调：每次成功入库（新增或置顶）后以最新详情调用。
/// 在监听线程上执行，实现必须自身保证线程安全并尽快返回。
pub type ClipUpsertSink = Arc<dyn Fn(ClipItemDetail) + Send + Sync>;

struct MonitorContext {
    core: CoreState,
    on_upsert: ClipUpsertSink,
    retry_count: u32,
}
...
impl ClipboardMonitor {
    pub fn start(core: CoreState, on_upsert: ClipUpsertSink) -> Result<(), AppError> {
        ...
        thread::spawn(move || unsafe {
            let (hwnd, hinstance) = match setup_monitor_window(&core, &on_upsert) {
                Ok(handles) => handles,
                Err(error) => {
...
unsafe fn handle_clipboard_update(hwnd: HWND) {
    ...
    // 退出期不再采集，避免访问正被销毁的资源
    if context.core.is_quitting() {
        return;
    }

    let settings = match context.core.current_settings() {
    ...
    match process_clipboard_change(&context.core, &context.on_upsert, source_app) {
        Ok(()) => {
            context.retry_count = 0;
            let _ = KillTimer(Some(hwnd), CLIPBOARD_RETRY_TIMER_ID);
        }
...
fn process_clipboard_change(
    ...
    if let Some(image) = read_image_from_clipboard()? {
        ...
        if let Some(detail) = HistoryService::ingest_image(core, prepared, source_app.clone())? {
            on_upsert(detail);
        }
        return Ok(());
    }
    ...
    match clipboard.get_text() {
        Ok(text) => {
            if let Some(detail) = HistoryService::ingest_text(core, &text, source_app)? {
                on_upsert(detail);
            }
        }
