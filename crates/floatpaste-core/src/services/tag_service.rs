...
impl TagService {
    ...
    pub fn rename_tag(state: &CoreState, old_name: &str, new_name: &str) -> Result<(), AppError> {
    ...
    pub fn delete_tag(state: &CoreState, name: &str) -> Result<(), AppError> {
    ...
    fn canonicalize(state: &CoreState, tag_names: &[String]) -> Result<Vec<String>, AppError> {
...
mod tests {
    ...
    fn normalize_tag_names_dedupes_case_variants() {
        let result = TagService::normalize_tag_names(&[
            "Work".to_string(),
            "work".to_string(),
            "个人".to_string(),
        ])
