...
fn tag_affected_item_ids(
    transaction: &Connection,
    tag_name: &str,
) -> Result<Vec<String>, AppError> {
    let mut statement =
        transaction.prepare("SELECT DISTINCT item_id FROM clip_item_tags WHERE tag_name = ?1")?;
...
"#;
    ...
    fn summary_includes_image_metadata_for_recent_and_search_results() {
        ...
        assert_eq!(
            recent_summary.image_path.as_deref(),
            Some("images/test-summary.png")
        );
        ...
        assert_eq!(
            search_summary.image_path.as_deref(),
            Some("images/test-summary.png")
        );
    ...
    fn search_relevance_ranks_favorited_and_tagged_items_first() {
        ...
        repository
            .set_item_tags("tagged-older", &["工作".to_string()])
            .unwrap();
        seed_item(
            &repository,
            "favorited-oldest",
        ...
        assert_eq!(
            ids,
            vec!["tagged-older", "favorited-oldest", "plain-recent"]
        );
    ...
    fn set_item_tags_nocase_variants_dedupe_to_single_row() {
        ...
        let rows: i64 = connection
            .query_row(
                "SELECT COUNT(*) FROM clip_item_tags WHERE item_id = ?1",
                [&id],
                |r| r.get(0),
            )
            .unwrap();
    ...
    fn rename_tag_syncs_fts_and_merges_case_variants() {
        ...
        repository
            .set_item_tags(&other, &["目标".to_string()])
            .unwrap();
    ...
    fn delete_tag_removes_associations_and_list_tags_excludes_deleted_items() {
        ...
        repository
            .set_item_tags(&kept, &["共享".to_string()])
            .unwrap();
        repository
            .set_item_tags(&removed, &["共享".to_string()])
            .unwrap();
    ...
    fn search_filters_by_tags_with_and_semantics() {
        ...
        repository
            ...
            .unwrap();
        repository
            .set_item_tags(&single, &["工作".to_string()])
            .unwrap();
        ...
        repository
            .set_item_tags(&english, &["Work".to_string()])
            .unwrap();
        let ci = repository
            .search(SearchQuery {
                keyword: String::new(),
    ...
    fn keyword_hits_cjk_tag_name_via_fts() {
        ...
        repository
            .set_item_tags(&id, &["设计稿".to_string()])
            .unwrap();
    ...
    fn list_recent_and_detail_carry_tags() {
        ...
        assert_eq!(
            summary.tags,
            vec!["乙标签".to_string(), "甲标签".to_string()]
        );
        let detail = repository.get_item_detail(&id).unwrap();
    ...
    fn purge_soft_deleted_removes_only_expired_rows() {
        ...
        let removed = repository
            .purge_soft_deleted(30 * 24 * 60 * 60 * 1000)
            .unwrap();
