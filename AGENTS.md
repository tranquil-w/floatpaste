...
## 构建、测试与开发命令
`pnpm install` 安装前端与 Tauri CLI 依赖。`pnpm dev` 启动浏览器预览，此模式会走 `src/bridge/mockBackend.ts`。`pnpm tauri dev` 启动桌面应用并连接真实 Rust 命令。`pnpm build` 先执行 TypeScript `--noEmit` 检查，再产出 Vite 包。需要桌面打包时使用 `pnpm tauri build`。新增 Rust 测试后，在 `src-tauri/` 下运行 `cargo test`。
...
## 测试指南
当前仓库没有独立前端测试框架，也没有专门的 `tests/` 目录，因此每次改动至少执行 `pnpm build`。Rust 逻辑变更建议补充单元测试，可就近写在模块内，或放入 `src-tauri/tests/`。测试名应描述行为，例如 `ingest_text_skips_self_write`。
