...
- `show_tooltip(app, x, y, html, theme)` — 设置内容、定位、显示
- `tooltip_ready(app, width, height)` — 前端 DOM 测量完成后回调，设置窗口实际大小和位置
- `hide_tooltip(app)` — 隐藏
...
- `show_tooltip(x: f64, y: f64, html: String, theme: String)` — 第一阶段：注入 HTML 并请求前端测量尺寸
- `tooltip_ready(width: u32, height: u32)` — 第二阶段：前端通过双层 `requestAnimationFrame` 测量 DOM 尺寸后回调，后端据此设置窗口大小和位置
- `hide_tooltip()` — 清除待处理状态并隐藏窗口
...
鼠标快速移动时，旧的 `outerPosition()` / `scaleFactor()` 异步查询可能在新请求之后才返回。前端使用自增 `requestId` + `invalidateTooltipRequest()` 模式防止过期结果触发 `showTooltip`：

1. 每次 `handleItemMouseMove` 调用前递增 `tooltipRequestIdRef`
2. `Promise.all` 返回后比较当前 `requestId` 与快照值
3. 不匹配时 `resolveTooltipShowPosition` 返回 `null`，跳过显示

`tooltipState.ts` 中的 `resolveTooltipShowPosition` 是纯函数，便于独立测试。

### 双阶段显示流程

由于 tooltip 窗口的实际尺寸在 HTML 注入前是未知的，采用了两阶段协议：

1. **`show_tooltip`**：将待显示位置写入全局 `PENDING_TOOLTIP_POS`，通过 `window.eval()` 注入 HTML 内容
2. **前端测量**：`tooltip.html` 在双层 `requestAnimationFrame` 回调中获取 DOM 的 `offsetWidth` / `offsetHeight`，调用 `tooltip_ready(width, height)`
3. **`on_tooltip_ready`**：消费 `PENDING_TOOLTIP_POS`，根据实际尺寸 + 4px padding 设置窗口大小，按坐标定位并显示

`hide_tooltip` 会主动清除 `PENDING_TOOLTIP_POS`，防止隐藏后迟到的 `tooltip_ready` 回调导致 tooltip 重新显示。
