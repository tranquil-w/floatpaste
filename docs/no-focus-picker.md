...
速贴窗口（Picker）在用户按下全局快捷键（默认 `Ctrl+Q`）时瞬间呼出，并且**尽量不抢占当前正在工作的目标软件（如 Word、代码编辑器）的输入焦点**。用户在速贴中可以使用会话快捷键（`Up / Down / Enter / Escape / 1-9`）、双击条目，或直接点击窗口外部结束本次速贴会话。速贴支持文本、图片和文件条目的展示与上屏，条目支持 Shift+Enter 次级上屏（按条目类型粘贴为路径文本），图片条目另支持悬浮大图预览。
...
5. **模拟粘贴**（`paste_flow.rs` / `paste_support.rs`）：
   ...
   - 次级上屏（Shift+Enter，按条目类型）：图片将所在文件路径写入剪贴板、文件将逐行路径列表作为文本写入剪贴板（文本暂同 Enter），模拟 `Ctrl+V`。
