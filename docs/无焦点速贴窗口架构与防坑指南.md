...
- **现象**：在桌面环境下，Picker 窗口四周出现明显的直角（通常为背景色或纯黑），导致前端设置的圆角视觉失效。
- **原因**：
  1. WebView2 所在的 `body` 或 `html` 标签若设置了不透明背景色，会遮挡 Tauri 窗口壳层的透明特性。
  2. 若 `body` 背景色为直角，则无论内部容器圆角多大，四个尖角都会漏出。
- **解决方案**：
  1. 强制设置 `html.window-picker body { background: transparent !important; }`，确保窗口根容器完全透明。
  2. 仅在内部的壳层容器（如 `.relative.flex.h-full...`）上应用背景色与圆角，从而利用 Tauri 的透明窗口特性实现完美的圆角浮窗效果。


---

## 4. 当前落地文件（截至 2026-03-11）
