...
2. **Win32 无焦点显示**：通过 `WS_EX_NOACTIVATE` 与 `ShowWindow(SW_SHOWNOACTIVATE)` 避免抢占焦点，并在必要时立即恢复目标窗口。
3. **会话期快捷键调度**：动态注册 Picker 会话快捷键，避免主快捷键与会话控制相互冲突。
4. **会话退出兜底**：通过鼠标低级钩子监听窗口外点击，并提供 `Tab` 切回资料库窗口的链路。
...
2. **无焦点显示**：调用 Windows API 给 Picker 打上 `WS_EX_NOACTIVATE` 扩展样式，并通过 `ShowWindow(SW_SHOWNOACTIVATE)` 显示窗口。
3. **保险式恢复焦点**：若存在目标窗口句柄，则在显示后立即尝试一次 `SetForegroundWindow(HWND)`，把焦点拉回原目标窗口。
4. **注册会话快捷键**：动态注册 `Up / Down / Enter / Escape / Tab / Digit1..Digit9`，接管本次速贴会话的键盘操作。
5. **启动鼠标外点监控**：注册 `WH_MOUSE_LL` 低级鼠标钩子，检测用户是否点在 Picker 外部。
6. **必要时隐藏 Manager**：如果本次是从资料库进入 Picker，则隐藏 Manager，只保留 Picker 作为当前可见主界面。
...
2. **隐藏 Picker**：通过 Tauri API 隐藏速贴窗口。
3. **延迟恢复目标**：休眠 `50ms`，等待窗口隐藏稳定后，再根据会话来源恢复目标应用或重新打开 Manager。
4. **模拟粘贴**：粘贴服务写入系统剪贴板，并利用 `SendInput` 模拟 `Ctrl+V`。
...
4. `ManagerShell` 在桌面运行时监听 `Escape` 并隐藏当前窗口；`PickerShell` 则负责按钮点击与浏览器预览模式下的本地按键 fallback。
